...
def self_defined_smap_loss(pred_ab, label_data, flag, sim_threshold):
    pred_a = pred_ab[:, 0].unsqueeze(1)
    pred_b = pred_ab[:, 1].unsqueeze(1)
    return ab_physics_loss_smap_v2(pred_ab, label_data, flag), EDS_loss(pred_a, sim_threshold, 10) #+ EDS_loss(pred_b, sim_threshold, 1)

def ab_physics_loss_smap_v2(pred_ab, label_data, flag): # sm = smap + a * (pij - p_bar) / p_sd + b
    # 获取smap
    ...
    for i in range(pred_ab.size(0)):
        a, b = pred_ab[i]
        # 计算ati_grid[i]的均值
        ati_mean = torch.sum(ati_grid[i])/torch.nonzero(ati_grid[i]).size(0)
        
        # 计算ati_grid[i]的标准差
        ati_sd = torch.sqrt(torch.sum(torch.mul(ati_grid[i] - ati_mean, ati_grid[i] - ati_mean))/torch.nonzero(ati_grid[i]).size(0))
        
        # 根据公式进行smap_downscaling的计算
        smap_downscaling = smap[i] + a * (ati_grid[i]-ati_mean)/ati_sd + b
        
        # 升尺度获得smap的预测值
        pred_smap[i] = torch.sum(smap_downscaling)/torch.nonzero(ati_grid[i]).size(0)
...
def ab_physics_loss_smap_v1(pred_ab, label_data, flag): # sm = a * pij + b
    # 获取smap
    smap = label_data['smap']
    # 获取ati_grid
    ati_grid = label_data['ati_grid']
